...
describe('git history graph model', () => {
  ...
  it('indexes merge parents once instead of rescanning a large history', () => {
    ...
    for (let index = 0; index < mergeCount; index += 1) {
      historyItems.push(trackedItem(`merge-${index}`, [`leaf-${index}`, `side-${index}`], reads))
      // Why both parents land as roots right after: a root closes only its own lane, so the
      // lanes stay bounded and this measures parent lookup, not lane growth.
      historyItems.push(trackedItem(`leaf-${index}`, [], reads))
      historyItems.push(trackedItem(`side-${index}`, [], reads))
    }
