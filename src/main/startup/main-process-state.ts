...
/** One Cmd+Q across several windows: every window confirms first, then all close, then quit. */
export type QuitConfirmationTransaction = {
...
export const mainProcessState = {
  /** Most recently opened main window; with several windows open, main-window-registry is the authority. */
  ...
  isQuitting: false,
  // Why a launch-time snapshot: project-scoped windows ride it too; flipping the setting mid-session would strand windows in a mixed model.
  experimentalMultiWindowEnabledAtStartup: false,
  quitConfirmedForAllWindows: false,
  activeQuitConfirmationTransaction: null as QuitConfirmationTransaction | null,
