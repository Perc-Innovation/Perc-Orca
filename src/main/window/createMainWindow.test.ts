...
describe('createMainWindow', () => {
      ...
      expect.objectContaining({
        webPreferences: expect.objectContaining({
          ...
          additionalArguments: [
            ...
            expect.stringMatching(/^--orca-browser-client-host-id=.+$/)
          ]
        })
      })
    )
  ...
  it('stamps the browser host id into the renderer that owns the guests, and strips it from a guest that carries one', () => {
    ...
    // Why: the window identity rides the same argv list; the host id is the last stamp.
    expect(stamped?.at(-1)).toEqual(formatBrowserClientHostIdArgument(getBrowserClientHostId()))
