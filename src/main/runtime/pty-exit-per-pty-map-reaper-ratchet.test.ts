...
const CLEARED_BY_REAPER_HELPER: Record<string, { helper: string; module: string }> = {
  ...
  // Window ownership (multi-window): the three indexes drop together so a respawned id never
  // inherits a dead PTY's window, claim or transient stamp.
  ptyOwnerWindowById: {
    helper: 'clearOwnerWindowForPty',
    module: 'src/main/runtime/orca-runtime-window-ownership-index.ts'
  },
  transientPtyOwnerWindowById: {
    helper: 'clearOwnerWindowForPty',
    module: 'src/main/runtime/orca-runtime-window-ownership-index.ts'
  },
  explicitPtyOwnerWindowById: {
    helper: 'clearOwnerWindowForPty',
    module: 'src/main/runtime/orca-runtime-window-ownership-index.ts'
  }
}
