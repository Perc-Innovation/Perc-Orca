...
import { emptyWindowGraphPublication } from './window-graph-publication'
...
export class OrcaRuntimeWithAttachWindow extends OrcaRuntimeWithNotifySshStateChanged {
  attachWindow(windowId: number): void {
    if (this.authoritativeWindowId === HEADLESS_RUNTIME_WINDOW_ID) {
      ...
      this.registerWindowGraphPublisher(windowId)
  ...
  // Why: every attached window may publish its own graph; an entry here is what admits it.
  private registerWindowGraphPublisher(windowId: number): void {
    if (!this.windowGraphPublications.has(windowId)) {
      this.windowGraphPublications.set(windowId, emptyWindowGraphPublication())
    }
  }
