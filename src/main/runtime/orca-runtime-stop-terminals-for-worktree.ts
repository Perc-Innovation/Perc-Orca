...
export class OrcaRuntimeWithStopTerminalsForWorktree extends OrcaRuntimeWithResolveTerminalSplitSourceAuthority {
  ...
  async stopTerminalsForWorktree(
    ...
    // Why: desktop IPC stops only what the calling window owns; a CLI/RPC stop has no window.
    const ptyIds = this.filterPtyIdsForOwnerWindow(
      this.collectWorktreePtyIds(worktree.id, hostFence),
      options.senderWindowId
    )
