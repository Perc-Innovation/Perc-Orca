...
export async function materializeRuntimeLocalWorktree<T>(args: {
  ...
  const meta = store.setWorktreeMeta(worktreeId, {
    ...
    ...(request.attachedReviews !== undefined
      ? { attachedReviews: normalizeAttachedReviews(request.attachedReviews) }
      : {}),
    ...(request.trackedBranches !== undefined
      ? { trackedBranches: normalizeTrackedBranches(request.trackedBranches) }
      : {}),
