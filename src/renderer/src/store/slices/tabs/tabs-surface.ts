...
/** Per-workspace remembered surface for workspaces that are not on screen.
 *  Why separate from buildActiveSurfacePatch: the global activeTabId/activeFileId
 *  fields describe the visible workspace, so a background workspace may only
 *  rewrite its own entry in the by-worktree maps. */
export function buildBackgroundSurfacePatch(
  state: Pick<
    AppState,
    | 'activeBrowserTabIdByWorktree'
    | 'activeFileIdByWorktree'
  ...
  const activeBrowserTabIdByWorktree = { ...state.activeBrowserTabIdByWorktree }
  ...
  for (const worktreeId of worktreeIds) {
    const derived = deriveActiveSurfaceForWorktree(state, worktreeId)
    activeBrowserTabIdByWorktree[worktreeId] = derived.activeBrowserTabId
    activeFileIdByWorktree[worktreeId] = derived.activeFileId
    activeTabIdByWorktree[worktreeId] = derived.activeTabId
    activeTabTypeByWorktree[worktreeId] = derived.activeTabType
  }
  return {
    activeBrowserTabIdByWorktree,
    activeFileIdByWorktree,
