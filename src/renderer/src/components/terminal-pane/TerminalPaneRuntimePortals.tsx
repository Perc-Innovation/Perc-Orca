...
/** Sibling of the mobile-driver portal for a pane whose PTY another window owns. */
export function TerminalPaneForeignWindowPortals({
  controller
}: {
  controller: TerminalPaneController
}): React.JSX.Element {
  const { chatLeafId, effectiveChatViewMode, managedPanes, paneTransportsRef } = controller
