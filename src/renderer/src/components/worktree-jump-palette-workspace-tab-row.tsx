...
export function WorktreeJumpPaletteWorkspaceTabRow({
  ...
  // Why the folder workspace fallback: it hangs off a project group, not a repo, and carries
  // its own host stamp — the host label is what tells a remote tab apart from a local one.
  const workspaceTabFolderWorkspaceId =
    !workspaceTabRepo && workspaceTabWorktree
      ? parseWorkspaceKey(workspaceTabWorktree.id)?.type === 'folder'
        ? workspaceTabWorktree.id.slice('folder:'.length)
        : null
      : null
  const workspaceTabHostSource =
    workspaceTabRepo ??
    (workspaceTabFolderWorkspaceId
      ? controller.folderWorkspaces.find((entry) => entry.id === workspaceTabFolderWorkspaceId)
      : undefined)
  const workspaceTabHostBadge = getPaletteHostBadge(
    workspaceTabHostSource,
