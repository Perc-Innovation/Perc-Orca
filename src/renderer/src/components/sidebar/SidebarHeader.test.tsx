...
vi.mock('./SidebarWorkspaceSelector', () => ({
  default: () => <span data-testid="workspace-selector" />
}))
...
describe('SidebarHeader', () => {
  ...
  it('names the window by its workspace instead of the grouping title', () => {
    // Why: the window is in one workspace at a time, so the selector takes the title's slot
    // whatever the grouping — the grouping never changes what the window is in.
    act(() => {
      root.render(<SidebarHeader onWorkspaceBoardMenuOpenChange={vi.fn()} />)
    })

    expect(container.querySelector('[data-testid="workspace-selector"]')).not.toBeNull()
    expect(container.querySelector('[data-sidebar-section-title="projects"]')).toBeNull()
    ...
    expect(container.querySelector('[data-testid="workspace-selector"]')).not.toBeNull()
    expect(container.querySelector('[data-sidebar-section-title="workspaces"]')).toBeNull()
  })
